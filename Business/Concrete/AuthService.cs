...
namespace Business.Concrete
{
    public class AuthService : IAuthService
    {
        ...
        public AuthService(IUnitOfWork unitOfWork, ITokenService tokenService, UserManager<User> userManager, SignInManager<User> signInManager, HttpContextManager httpContextManager, IMapper mapper)
        {
        ...
        public async Task<LoginResponse> LoginAsync(LoginRequest loginRequest, CancellationToken cancellationToken = default)
        {
            ...
            if (user == null) throw new BusinessException("Kullanıcı adı sistemde tanımlı değil.", description: $"Requester user name : {loginRequest.UserName}");

            bool isPasswordValid = await _userManager.CheckPasswordAsync(user, loginRequest.Password);
            if (!isPasswordValid) throw new BusinessException("Girdiğiniz şifre doğru değil.", description: $"Requester user name: {loginRequest.UserName}");
            ...
            if (string.IsNullOrEmpty(ipAddress))
                throw new GeneralException("Ip address could not found for login.", description: $"Requester user name address: {loginRequest.UserName}");
        ...
        public async Task<SignUpResponse> SignUpAsync(SignUpRequest signUpRequest, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                User user = await ProcessSignup(signUpRequest);
                IList<string> roles = await _userManager.GetRolesAsync(user);
                IList<Claim> claims = await GetClaimsAsync(user, roles);
                AccessToken accessToken = _tokenService.GenerateAccessToken(claims);
                RefreshToken refreshToken = _tokenService.GenerateRefreshToken(user);
        ...
        [ExceptionHandler, Validation(typeof(RefreshAuthRequest))]
        public async Task<RefreshAuthResponse> RefreshAuthAsync(RefreshAuthRequest refreshAuthRequest, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                if (user == null)
                    throw new GeneralException("User cannot found for refresh auth!", description: $"Requester userId: {refreshAuthRequest.UserId}");
                string? ipAddress = _httpContextManager.GetClientIp();
                if (string.IsNullOrEmpty(ipAddress))
                    throw new GeneralException("Ip address could not readed for refresh auth.");
                DateTime nowOnUtc = DateTime.UtcNow;
                ICollection<RefreshToken>? refreshTokens = await _unitOfWork.RefreshTokens.GetAllAsync(where: f => f.UserId == refreshAuthRequest.UserId && f.TTL > 0 && f.ExpirationUtc > nowOnUtc && f.IpAddress.ToLowerInvariant().Trim() == ipAddress.ToLowerInvariant().Trim(), cancellationToken: cancellationToken);
                if (refreshTokens == null)
                    throw new GeneralException("There is no available refresh token.");
        ...
        public async Task LoginWebBaseAsync(LoginRequest loginRequest, CancellationToken cancellationToken = default)
        {
            ...
            var result = await _signInManager.PasswordSignInAsync(user, loginRequest.Password, isPersistent: loginRequest.RememberMe, lockoutOnFailure: false);
            if (result.IsLockedOut)
            {
                throw new BusinessException("Bu hesaba erişim engeli getirildi.", description: $"Requester user name : {loginRequest.UserName}");
            }
            else if (!result.Succeeded)
            {
                throw new BusinessException("İşlem geçersiz lütfen bilgileriniz kontrol ediniz.", description: $"Requester user name : {loginRequest.UserName}");
            }
        }
        ...
        public async Task SignUpWebBaseAsync(SignUpRequest signUpRequest, CancellationToken cancellationToken = default)
        {
            ...
            {
                throw new BusinessException("Bu hesapta erişim engeli bulunmaktadır.", description: $"Requester user name : {signUpRequest.UserName}");
            }
            else if (!resultSignIn.Succeeded)
            {
                throw new BusinessException("İşlem geçersiz lütfen bilgileriniz kontrol ediniz.", description: $"Requester user name : {signUpRequest.UserName}");
            }
        }
        #endregion

        #region HELPER METHODS
        private async Task<User> ProcessSignup(SignUpRequest signUpRequest)
        {
            var userExist = await _userManager.FindByNameAsync(signUpRequest.UserName);
            if (userExist != null)
                throw new BusinessException("Kullanıcı adı sistemde zaten mevcut.", description: $"Requester user name : {signUpRequest.UserName}");
            var user = _mapper.Map<User>(signUpRequest);
            var result = await _userManager.CreateAsync(user, signUpRequest.Password);
            if (!result.Succeeded)
                throw new GeneralException(string.Join("\n", result.Errors.Select(e => e.Description)), description: $"User cannot be created. Requester user name: {signUpRequest.UserName}");
            var roleResult = await _userManager.AddToRoleAsync(user, "User");
            if (!roleResult.Succeeded)
                throw new GeneralException("Failed to assign role.", description: $"Requester user name: {signUpRequest.UserName}");
            return user;
        }
